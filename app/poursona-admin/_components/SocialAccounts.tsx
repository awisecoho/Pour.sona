...
const label: React.CSSProperties = { color: '#C9A84C', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', display: 'block', marginBottom: 8 }
const inp: React.CSSProperties = { width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(201,168,76,.2)', borderRadius: 8, color: '#F5ECD7', fontFamily: 'Georgia, serif', fontSize: 13, outline: 'none', boxSizing: 'border-box' }
...
export default function SocialAccounts() {
  ...
  if (loading) return <div style={{ color: '#9a8a64' }}>Loading social accounts…</div>
  ...
  return (
    <div>
      ...
      {/* Connect platforms */}
      <div style={card}>
        <div style={{ color: '#F5ECD7', fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Link Accounts</div>
        <div style={{ color: '#9a8a64', fontSize: 12, marginBottom: 16 }}>Connect via OAuth to enable posting, or add an account manually to track &amp; select it for research.</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12 }}>
          {platforms.map(p => (
            <div key={p.id} style={{ border: '1px solid rgba(201,168,76,.15)', borderRadius: 10, padding: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: 18 }}>{p.icon}</span>
                <span style={{ color: '#F5ECD7', fontSize: 14, fontWeight: 700 }}>{p.label}</span>
              </div>
              <div style={{ fontSize: 11, color: '#9a8a64', lineHeight: 1.5, marginBottom: 10, minHeight: 32 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {p.configured ? (
                  <a href={'/api/poursona-admin/social/connect/' + p.id}
                    style={{ flex: 1, textAlign: 'center', padding: '9px 0', background: 'linear-gradient(135deg,#C9A84C,#a07830)', borderRadius: 8, color: '#060403', textDecoration: 'none', fontSize: 12, fontWeight: 700 }}>
                    Connect →
                  </a>
                ) : (
                  <span style={{ flex: 1, textAlign: 'center', padding: '9px 0', background: 'rgba(255,255,255,.04)', borderRadius: 8, color: '#7a6a44', fontSize: 12 }}>Not configured</span>
                )}
                <button onClick={() => { setAddFor(addFor === p.id ? null : p.id); setAddName(''); setAddUrl('') }}
                  style={{ padding: '9px 12px', background: 'rgba(201,168,76,.08)', border: '1px solid rgba(201,168,76,.2)', borderRadius: 8, color: '#C9A84C', fontSize: 12, cursor: 'pointer', fontFamily: 'Georgia, serif' }}>
                  + Add
                </button>
              </div>
              {addFor === p.id && (
                <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <button onClick={() => addManual(p.id)} style={{ padding: '8px', background: 'rgba(94,207,138,.12)', border: '1px solid rgba(94,207,138,.3)', borderRadius: 8, color: '#5ecf8a', fontSize: 12, cursor: 'pointer', fontFamily: 'Georgia, serif' }}>Save account</button>
                </div>
              )}
            </div>
          ))}
      ...
      <div style={card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div style={{ color: '#F5ECD7', fontSize: 14, fontWeight: 700 }}>Linked Accounts ({accounts.length})</div>
          <div style={{ color: '#9a8a64', fontSize: 12 }}>{selectedCount} selected</div>
        </div>
        {accounts.length === 0 ? (
          <div style={{ color: '#9a8a64', fontSize: 13 }}>No accounts yet. Connect or add one above.</div>
        ) : accounts.map(a => (
          <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid rgba(201,168,76,.06)' }}>
            <input type="checkbox" checked={a.selected} onChange={e => toggle(a.id, e.target.checked)} style={{ width: 16, height: 16, accentColor: '#C9A84C', cursor: 'pointer' }} />
            <span style={{ fontSize: 18 }}>{PLATFORM_ICON[a.platform] || '🔗'}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ color: '#F5ECD7', fontSize: 14 }}>{a.display_name || a.platform}</div>
              <div style={{ color: '#9a8a64', fontSize: 11, textTransform: 'capitalize' }}>{a.platform}</div>
            </div>
            <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11,
              background: a.status === 'connected' ? 'rgba(94,207,138,.12)' : 'rgba(201,168,76,.1)',
              color: a.status === 'connected' ? '#5ecf8a' : '#C9A84C',
              border: '1px solid ' + (a.status === 'connected' ? 'rgba(94,207,138,.3)' : 'rgba(201,168,76,.25)') }}>
      ...
      {/* Compose & post */}
      <div style={card}>
        <div style={{ color: '#F5ECD7', fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Compose &amp; Post</div>
        <div style={{ color: '#9a8a64', fontSize: 12, marginBottom: 12 }}>Publishes to the {selectedCount} selected account{selectedCount !== 1 ? 's' : ''}. Manual (non-OAuth) accounts are skipped with a reason.</div>
        ...
        <button onClick={post} disabled={posting || selectedCount === 0 || !composeBody.trim()}
          style={{ padding: '12px 22px', borderRadius: 8, border: 'none', cursor: posting || selectedCount === 0 ? 'not-allowed' : 'pointer', fontFamily: 'Georgia, serif', fontSize: 13, fontWeight: 700, background: selectedCount === 0 ? 'rgba(201,168,76,.3)' : 'linear-gradient(135deg,#C9A84C,#a07830)', color: '#060403', opacity: posting ? .6 : 1 }}>
