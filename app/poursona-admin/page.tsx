...
function StatCard({ label, value, sub, color }: { label: string; value: string | number; sub?: string; color?: string }) {
  return (
    <div style={{ background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.15)', borderRadius: 12, padding: '18px 20px' }}>
      <div style={{ color: '#3A3450', fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', marginBottom: 6 }}>{label}</div>
      <div style={{ color: color || '#F5F2E8', fontSize: 28, fontWeight: 700 }}>{value}</div>
      {sub && <div style={{ color: '#3A3450', fontSize: 11, marginTop: 4 }}>{sub}</div>}
    </div>
  )
}
  ...
  return (
    <div>
          ...
          {data.map((d, i) => {
            ...
            return (
              <g key={d.day}>
                ...
                {d.orders > 0 && <rect x={x} y={72 - oH} width={barW} height={oH} fill="rgba(97,42,134,0.65)" rx={1} />}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 2 }}>
        <span style={{ color: '#3A3450', fontSize: 9 }}>{data[0]?.day.slice(5)}</span>
        <span style={{ color: '#3A3450', fontSize: 9 }}>{data[data.length - 1]?.day.slice(5)}</span>
      </div>
      <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#3A3450', fontSize: 10 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: 'rgba(255,255,255,0.08)', display: 'inline-block' }} />Sessions</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#D67A31', fontSize: 10 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: 'rgba(97,42,134,0.65)', display: 'inline-block' }} />Orders</span>
...
export default function AppCommandCenter() {
  ...
  const inp: React.CSSProperties = { width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(97,42,134,.2)', borderRadius: 8, color: '#F5F2E8', fontFamily: 'var(--font-inter), system-ui, sans-serif', fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }
  const label: React.CSSProperties = { color: '#D67A31', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', display: 'block', marginBottom: 5 }
  const btn = (v?: string): React.CSSProperties => ({ padding: '10px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-inter), system-ui, sans-serif', fontSize: 12, fontWeight: 700, background: v === 'danger' ? 'rgba(255,100,100,.15)' : v === 'green' ? 'rgba(94,207,138,.15)' : v === 'outline' ? 'rgba(97,42,134,.08)' : 'linear-gradient(135deg,#D67A31,#612A86)', color: v === 'danger' ? '#e07070' : v === 'green' ? '#5ecf8a' : v === 'outline' ? '#D67A31' : '#12111A', ...(v === 'outline' ? { border: '1px solid rgba(97,42,134,.25)' } : {}) })
  const tabBtn = (t: Tab): React.CSSProperties => ({ padding: '9px 16px', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-inter), system-ui, sans-serif', fontSize: 12, fontWeight: 700, background: tab === t ? 'rgba(97,42,134,.15)' : 'transparent', color: tab === t ? '#D67A31' : '#3A3450', borderBottom: tab === t ? '2px solid #D67A31' : '2px solid transparent', borderRadius: '8px 8px 0 0' })

  return (
    <div>
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ color: '#D67A31', fontSize: 10, letterSpacing: '.3em', textTransform: 'uppercase', marginBottom: 4 }}>Poursona Internal</div>
          <div style={{ color: '#F5F2E8', fontSize: 26, fontWeight: 700 }}>App Command Center</div>
          <div style={{ color: '#3A3450', fontSize: 13, marginTop: 4 }}>{totalVendors} retailers · {totalSessions} sessions · {convRate} conversion</div>
        </div>
        <Link href="/poursona-admin/onboard" style={{ display: 'inline-block', padding: '11px 22px', background: 'linear-gradient(135deg,#D67A31,#612A86)', borderRadius: 10, color: '#12111A', textDecoration: 'none', fontSize: 13, fontWeight: 700 }}>
          + Onboard Retailer
        </Link>
      </div>

      <div style={{ display: 'flex', gap: 2, marginBottom: 20, borderBottom: '1px solid rgba(97,42,134,.1)', flexWrap: 'wrap' }}>
      ...
      {tab === 'dashboard' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(148px, 1fr))', gap: 12, marginBottom: 24 }}>
            ...
            <StatCard label="On Trial" value={trialCount} color="#D67A31" sub={expiredCount > 0 ? expiredCount + ' expired' : 'all current'} />
            <StatCard label="Total Sessions" value={totalSessions.toLocaleString()} sub={totalOrders + ' orders'} />
            <StatCard label="Expired Trials" value={expiredCount} color={expiredCount > 0 ? '#e07070' : '#3A3450'} sub="need follow-up" />
            ...
            <div style={{ background: 'rgba(255,100,100,.06)', border: '1px solid rgba(255,100,100,.2)', borderRadius: 12, padding: '16px 20px', marginBottom: 20 }}>
              ...
              {expiredList.map(r => (
                <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(255,100,100,.1)' }}>
                  <div><span style={{ color: '#F5F2E8', fontSize: 13 }}>{r.name}</span><span style={{ color: '#3A3450', fontSize: 11, marginLeft: 10 }}>expired {r.trial_ends_at ? new Date(r.trial_ends_at).toLocaleDateString() : ''}</span></div>
                  <Link href={'/poursona-admin/retailer/' + r.id} style={{ color: '#D67A31', fontSize: 12, textDecoration: 'none' }}>Extend →</Link>
                </div>
              ))}
            </div>
          )}
          <div style={{ background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.15)', borderRadius: 14, padding: '20px 24px', marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <div style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700 }}>System Analytics</div>
              {analytics?.range && (
                <div style={{ color: '#3A3450', fontSize: 10 }}>{analytics.range.chartFrom} → {analytics.range.chartTo}</div>
              )}
            </div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
              <input
                ...
                style={{ background: '#161423', border: '1px solid rgba(97,42,134,.2)', borderRadius: 8, color: '#F5F2E8', padding: '6px 10px', fontSize: 12 }}
              />
              <span style={{ color: '#3A3450', fontSize: 11 }}>to</span>
              <input
                type="date"
                value={analyticsTo}
              ...
              <button
                ...
                style={{ padding: '6px 14px', background: '#D67A31', color: '#161423', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
              >
              ...
              {(analyticsFrom || analyticsTo) && (
                <button
                  ...
                  style={{ padding: '6px 14px', background: 'transparent', color: '#3A3450', border: '1px solid rgba(97,42,134,.2)', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}
                >
                  Clear
                </button>
              )}
              <button
                ...
                style={{ padding: '6px 14px', background: 'transparent', color: '#D67A31', border: '1px solid rgba(97,42,134,.3)', borderRadius: 8, fontSize: 12, cursor: 'pointer', marginLeft: 'auto' }}
              >
                ↓ Export CSV
              </button>
            </div>
            {analytics && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
                <div style={{ background: '#060402', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ color: '#3A3450', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 4 }}>Sessions</div>
                  <div style={{ color: '#F5F2E8', fontSize: 22, fontWeight: 700 }}>{analytics.summary.totalSessions.toLocaleString()}</div>
                </div>
                <div style={{ background: '#060402', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ color: '#3A3450', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 4 }}>Orders</div>
                  <div style={{ color: '#D67A31', fontSize: 22, fontWeight: 700 }}>{analytics.summary.totalOrders.toLocaleString()}</div>
                </div>
                <div style={{ background: '#060402', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ color: '#3A3450', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 4 }}>Conversion</div>
                  <div style={{ color: '#F5F2E8', fontSize: 22, fontWeight: 700 }}>{analytics.summary.conversionRate}%</div>
                </div>
                <div style={{ background: '#060402', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ color: '#3A3450', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 4 }}>Active Vendors</div>
            ...
            {analyticsLoading && !analytics && (
              <div style={{ color: '#3A3450', fontSize: 12, textAlign: 'center', padding: '20px 0' }}>Loading analytics…</div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            {[{ label: 'Onboard', href: '/poursona-admin/onboard', icon: '+' }, { label: 'Team', href: '/poursona-admin/team', icon: '👥' }, { label: 'System Check', href: '/poursona-admin/system-check', icon: '⚙' }].map(({ label, href, icon }) => (
              <Link key={href} href={href} style={{ padding: '16px 12px', background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.15)', borderRadius: 12, color: '#D67A31', textDecoration: 'none', textAlign: 'center', display: 'block' }}>
        ...
        <div>
            ...
            return (
              <div key={r.id} style={{ ...card, marginBottom: 0, opacity: r.active ? 1 : 0.55 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 10, background: r.brand_color || '#D67A31', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>{ICONS[r.vertical] || '✦'}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ color: '#F5F2E8', fontSize: 15, fontWeight: 700 }}>{r.name}</div>
          ...
          {pagination && pagination.totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 20, justifyContent: 'center' }}>
              ...
              <span style={{ color: '#3A3450', fontSize: 12 }}>{retailerPage} / {pagination.totalPages}</span>
              <button
                onClick={() => loadRetailers(retailerPage + 1, retailerSearch)}
      ...
      {tab === 'accounting' && (
        <div>
          {!accounting ? (
            <div style={{ color: '#3A3450', textAlign: 'center', padding: '40px 0' }}>
              <div style={{ color: '#D67A31', marginBottom: 8 }}>Loading accounting data…This requires auth.</div>
            ...
            <>
              ...
              <div style={card}>
                <div style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Vendor Billing</div>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead><tr style={{ borderBottom: '1px solid rgba(97,42,134,.15)' }}>{['Vendor', 'Plan', 'Status', 'MRR', 'Credits', 'Since'].map(h => <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: '#3A3450', fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', fontWeight: 700 }}>{h}</th>)}</tr></thead>
                    <tbody>{(accounting.vendors || []).map((v: any) => ( <tr key={v.id} style={{ borderBottom: '1px solid rgba(97,42,134,.06)' }}><td style={{ padding: '10px', color: '#F5F2E8' }}>{v.name}</td><td style={{ padding: '10px', color: '#6A6080', textTransform: 'capitalize' }}>{v.subscription_tier || '—'}</td><td style={{ padding: '10px' }}><span style={{ color: v.subscription_status === 'active' ? '#5ecf8a' : '#D67A31', textTransform: 'capitalize' }}>{v.subscription_status || 'trial'}</span></td><td style={{ padding: '10px', color: v.mrr ? '#5ecf8a' : '#3A3450' }}>{v.mrr ? '$' + v.mrr : '—'}</td><td style={{ padding: '10px', color: '#3A3450' }}>{v.credit_balance ? '$' + v.credit_balance : '—'}</td><td style={{ padding: '10px', color: '#3A3450' }}>{v.created_at ? new Date(v.created_at).toLocaleDateString() : '—'}</td></tr> ))}</tbody>
                  </table>
                </div>
              </div>
            </>
      ...
      {tab === 'promos' && (
        <div>
          <div style={card}>
            <div style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700, marginBottom: 16 }}>Create Promo Code</div>
          ...
          <div style={card}>
            <div style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700, marginBottom: 14 }}>All Promos ({promos.length})</div>
            {promos.length === 0 ? <div style={{ color: '#3A3450', fontSize: 13 }}>No promo codes yet.</div> : promos.map(p => (
              <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(97,42,134,.06)' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700, letterSpacing: '.05em' }}>{p.code}</span>
                  ...
                  <div style={{ color: '#3A3450', fontSize: 12, marginTop: 4 }}>{p.type === 'percent' ? p.value + '% off' : p.type === 'fixed' ? '$' + p.value + ' off' : p.type === 'trial_days' ? p.value + ' free days' : '$' + p.value + ' credit'}{p.max_uses ? ' · ' + (p.uses_count || 0) + '/' + p.max_uses : p.uses_count ? ' · ' + p.uses_count + ' uses' : ''}{p.description ? ' · ' + p.description : ''}</div>
                </div>
      ...
      {tab === 'pipeline' && (
        <div>
          <div style={{ marginBottom: 20 }}>
            <div style={{ color: '#F5F2E8', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Prospect Pipeline</div>
            <div style={{ color: '#A89FB8', fontSize: 13 }}>Find independent venues, qualify them with AI web search, and generate a personalised outreach message in one click. Save the ones worth following up in the <strong style={{ color: '#D67A31' }}>Leads</strong> tab.</div>
          </div>
          <ProspectPipeline />
        </div>
      )}
      ...
      {tab === 'leads' && (
        <div>
          <div style={{ marginBottom: 20 }}>
            <div style={{ color: '#F5F2E8', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Leads</div>
            <div style={{ color: '#A89FB8', fontSize: 13 }}>Saved prospects with status, contact channels, and activity history. The Gmail Email button on the Pipeline auto-logs an &ldquo;email sent&rdquo; activity on the matching lead.</div>
          </div>
          <LeadsManager />
        </div>
      )}
      ...
      {tab === 'social' && (
        <div>
          <div style={{ marginBottom: 20 }}>
            <div style={{ color: '#F5F2E8', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Social Accounts</div>
            <div style={{ color: '#A89FB8', fontSize: 13 }}>Link Facebook/Instagram, LinkedIn, and X. Select one or more accounts for research and posting through the marketing agent.</div>
          </div>
          <SocialAccounts />
        </div>
      )}
      ...
      {tab === 'customers' && (
        <div>
          ...
          <div style={card}>
            <div style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Customer Profiles ({customers.length})</div>
            {customers.length === 0 ? <div style={{ color: '#3A3450', fontSize: 13 }}>No customer profiles found.</div> : customers.map((c: any) => (
              <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '12px 0', borderBottom: '1px solid rgba(97,42,134,.06)' }}>
                <div>
                  <div style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700 }}>{c.name || '(no name)'}</div>
                  <div style={{ color: '#6A6080', fontSize: 12, marginTop: 2 }}>{c.email}</div>
                  <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
                    {c.opted_in_promos && <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 10, background: 'rgba(97,42,134,.1)', color: '#D67A31', border: '1px solid rgba(97,42,134,.2)' }}>promo</span>}
                    ...
                    {c.total_visits > 0 && <span style={{ color: '#3A3450', fontSize: 11 }}>{c.total_visits} visit{c.total_visits !== 1 ? 's' : ''}</span>}
                  </div>
                </div>
                <div style={{ color: '#3A3450', fontSize: 11, textAlign: 'right', flexShrink: 0, marginLeft: 12 }}>{c.created_at ? new Date(c.created_at).toLocaleDateString() : ''}</div>
              </div>
            ))}
          </div>
        </div>
