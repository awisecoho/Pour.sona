...
export default function RetailerDetail() {
  ...
  async function load() {
    const [rRes, pRes, fRes, sRes] = await Promise.all([
      ...
      sb.from('products').select('*').eq('retailer_id', id).order('sort_order').limit(30),
  ...
  const s = {
    ...
    tab: (active: boolean) => ({ padding: '9px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'Georgia, serif', fontSize: 12, fontWeight: 700, background: active ? 'rgba(201,168,76,.15)' : 'transparent', color: active ? '#C9A84C' : '#4a3a1a', borderBottom: active ? '2px solid #C9A84C' : '2px solid transparent' } as React.CSSProperties),
  }
    ...
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          {saving && <div style={{ color: '#C9A84C', fontSize: 11, marginLeft: 'auto' }}>Saving…</div>}
        </div>
      </div>

      {/* Quick actions */}
      <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' as const }}>
        <a href={'/r/' + retailer.slug} target="_blank" style={{ padding: '10px 18px', background: 'linear-gradient(135deg,#C9A84C,#a07830)', borderRadius: 8, color: '#060403', textDecoration: 'none', fontSize: 12, fontWeight: 700 }}>↗ Preview Experience</a>
        <a href={'/admin?retailer=' + retailer.id} target="_blank" style={{ padding: '10px 18px', background: 'rgba(201,168,76,.1)', border: '1px solid rgba(201,168,76,.2)', borderRadius: 8, color: '#C9A84C', textDecoration: 'none', fontSize: 12, fontWeight: 700 }}>⊞ Vendor Admin</a>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1px solid rgba(201,168,76,.1)', paddingBottom: 0 }}>
      ...
      {tab === 'invite' && (
        <div style={s.card}>
          ...
          <div style={{ color: '#4a3a1a', fontSize: 13, marginBottom: 20 }}>Send a login link to the vendor so they can access their admin portal.</div>
          ...
          <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid rgba(201,168,76,.1)' }}>
            <div style={{ color: '#4a3a1a', fontSize: 11, marginBottom: 6 }}>Direct customer link</div>
            <div style={{ color: '#C9A84C', fontSize: 13, wordBreak: 'break-all' as const }}>pour-sona.vercel.app/r/{retailer.slug}</div>
          </div>
        </div>
      )}
    </div>
