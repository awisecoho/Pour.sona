...
function useTheme(retailer: Retailer | null) {
  return useMemo(() => {
    ...
    return { primary, rgbStr, onPrimary: luminance > 0.55 ? '#0a0603' : '#F5ECD7' }
  }, [retailer])
}
  ...
  return (
    <div style={{ minHeight:'100vh',background:'#080604',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:14 }}>
      <div style={{ display:'flex',gap:7 }}>
        {[0,1,2].map(i=><div key={i} style={{ width:7,height:7,borderRadius:'50%',background:'#C9A84C',animation:`ldot 1.3s ease-in-out ${i*.18}s infinite` }}/>)}
      </div>
...
function ClaimBanner({ draftId, theme, font, visible }: { draftId: string; theme: ReturnType<typeof useTheme>; font: string; visible: boolean }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: '#d4c8a8', fontSize: 13, fontFamily: `'${font}', Georgia, serif`, lineHeight: 1.4, flex: 1, minWidth: 0 }}>
        <span style={{ color: theme.primary, fontWeight: 700 }}>This is YOUR guide.</span>{' '}
        <span style={{ color: '#9a8a6a' }}>Claim it free — 14-day trial.</span>
      </div>
      <button
        onClick={claim}
        style={{
          ...
          fontFamily: `'${font}', Georgia, serif`, letterSpacing: '.08em',
...
function WelcomeScreen({ retailer, onStart, theme, font, draftId }: { retailer: Retailer; onStart:()=>void; theme: ReturnType<typeof useTheme>; font: string; draftId: string }) {
  ...
  return (
    <div style={{ minHeight:'100vh',background:'linear-gradient(170deg,#080604 0%,#0c1208 100%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 28px',textAlign:'center',position:'relative',overflow:'hidden' }}>
      ...
      {/* Demo badge */}
      <div style={{ position:'absolute',top:20,right:20,background:`rgba(${theme.rgbStr},.12)`,border:`1px solid rgba(${theme.rgbStr},.3)`,borderRadius:20,padding:'5px 12px',color:theme.primary,fontSize:11,fontFamily:`'${font}',Georgia,serif`,letterSpacing:'.1em' }}>
        ✦ Your Preview
      </div>
      ...
      <div style={{ color:'#F5ECD7',fontSize:34,fontWeight:700,fontFamily:`'${font}',Georgia,serif`,lineHeight:1.2,marginBottom:6,letterSpacing:'-.3px' }}>{retailer.name}</div>
      {retailer.location && <div style={{ color:`rgba(${theme.rgbStr},.6)`,fontSize:13,fontFamily:`'${font}',Georgia,serif`,marginBottom:4 }}>{retailer.location}</div>}
      {retailer.tagline && <div style={{ color:'#6a5a3a',fontSize:15,fontStyle:'italic',fontFamily:`'${font}',Georgia,serif` }}>{retailer.tagline}</div>}
      ...
      <button onClick={onStart} style={{ background:theme.primary,border:'none',borderRadius:50,padding:'15px 0',width:'100%',maxWidth:280,color:theme.onPrimary,fontSize:14,fontWeight:700,letterSpacing:'.15em',cursor:'pointer',fontFamily:`'${font}',Georgia,serif`,boxShadow:`0 8px 32px rgba(${theme.rgbStr},.25)`,marginBottom:14 }}>
        Try it — Find My {noun}
      </button>

      <button onClick={claim} style={{ background:'transparent',border:`1px solid rgba(${theme.rgbStr},.3)`,borderRadius:50,padding:'11px 0',width:'100%',maxWidth:280,color:theme.primary,fontSize:13,fontWeight:600,letterSpacing:'.1em',cursor:'pointer',fontFamily:`'${font}',Georgia,serif` }}>
        Claim this guide →
      </button>
      <div style={{ marginTop:10,color:'#3a2a0a',fontSize:11 }}>14-day free trial · no credit card</div>
    </div>
  )
}
  ...
  return (
    <div style={{ overflowX:'auto',WebkitOverflowScrolling:'touch',paddingBottom:4,marginTop:8,maxWidth:'100%' }}>
      <div style={{ display:'flex',gap:8,paddingRight:4,width:'max-content' }}>
        {chips.map(chip=>(
          <button key={chip} onClick={()=>onSelect(chip)} style={{ background:`rgba(${theme.rgbStr},.06)`,border:`1px solid rgba(${theme.rgbStr},.28)`,borderRadius:24,padding:'9px 18px',color:theme.primary,fontSize:14,fontFamily:`'${font}',Georgia,serif`,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0 }}>{chip}</button>
        ))}
      </div>
    </div>
  )
...
function DemoRecommendationCard({ rec,retailer,ctas,onTryAnother,theme,font,draftId }:{ rec:BlendRecommendation;retailer:Retailer;ctas:{primary:string;secondary:string};onTryAnother:()=>void;theme:ReturnType<typeof useTheme>;font:string;draftId:string }) {
  ...
  return (
    <div style={{ marginTop:20,background:'linear-gradient(155deg,#100c06 0%,#0a0e08 100%)',border:`1px solid rgba(${theme.rgbStr},.22)`,borderRadius:20,overflow:'hidden',boxShadow:`0 0 60px rgba(${theme.rgbStr},.07),0 24px 48px rgba(0,0,0,.5)`,animation:'recReveal .45s cubic-bezier(.22,1,.36,1) both' }}>

      {/* Flavor chips */}
      {(rec.flavorProfile||[]).length>0 && (
        <div style={{ padding:'14px 20px 0',display:'flex',flexWrap:'wrap',gap:6 }}>
          {(rec.flavorProfile||[]).map(f=><span key={f} style={{ background:`rgba(${theme.rgbStr},.1)`,border:`1px solid rgba(${theme.rgbStr},.2)`,borderRadius:20,padding:'4px 13px',color:theme.primary,fontSize:12,fontFamily:`'${font}',Georgia,serif` }}>{f}</span>)}
        </div>
      )}

      {/* Name */}
      <div style={{ padding:'12px 20px 0' }}>
        <div style={{ color:`rgba(${theme.rgbStr},.5)`,fontSize:10,letterSpacing:'.28em',textTransform:'uppercase',marginBottom:6,fontFamily:`'${font}',Georgia,serif` }}>{retailer.name} · Your {noun}</div>
        <div style={{ color:'#F5ECD7',fontFamily:`'${font}',Georgia,serif`,fontSize:26,fontWeight:700,lineHeight:1.12,marginBottom:4 }}>{rec.recommendationName||rec.blendName}</div>
        {rec.tagline && <div style={{ color:theme.primary,fontFamily:`'${font}',Georgia,serif`,fontSize:14,fontStyle:'italic',opacity:.9 }}>{rec.tagline}</div>}
      </div>
      ...
      <div style={{ padding:'18px 20px 20px',display:'flex',flexDirection:'column',gap:10 }}>
        <button onClick={claim} style={{ width:'100%',padding:'16px 12px',borderRadius:14,background:theme.primary,border:'none',cursor:'pointer',color:theme.onPrimary,fontSize:14,fontWeight:700,letterSpacing:'.08em',fontFamily:`'${font}',Georgia,serif`,boxShadow:`0 8px 28px rgba(${theme.rgbStr},.35)` }}>
          ✦ Claim this guide — 14-day free trial
        </button>
        <button onClick={onTryAnother} style={{ padding:'12px',borderRadius:14,background:'transparent',border:`1px solid rgba(${theme.rgbStr},.28)`,cursor:'pointer',color:theme.primary,fontSize:12,fontWeight:600,letterSpacing:'.06em',fontFamily:`'${font}',Georgia,serif` }}>
          {ctas.secondary}
        </button>
      </div>
    </div>
...
export default function DemoPage({ params }: { params: { draftId: string } }) {
  ...
  if (notFound || !retailer) return (
    <div style={{ minHeight:'100vh',background:'#080604',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:10 }}>
      ...
      <div style={{ color:'#d4c8a8',fontFamily:'Georgia,serif',fontSize:18 }}>Demo not found</div>
      <a href="/signup" style={{ color:'#C9A84C',fontFamily:'Georgia,serif',fontSize:14,marginTop:8 }}>Start a new signup →</a>
    </div>
  )
  ...
  return (
    <div style={{ minHeight:'100vh',background:'#080604',display:'flex',flexDirection:'column',fontFamily:`'${font}',Georgia,serif`,paddingTop: showBanner ? 52 : 0 }}>
      ...
      <div style={{ flex:1,overflowY:'auto',padding:'20px 20px 0',maxWidth:520,margin:'0 auto',width:'100%' }}>
        {messages.map((m,i)=>(
          <div key={i} style={{ marginBottom:16,display:'flex',flexDirection:'column',alignItems:m.role==='user'?'flex-end':'flex-start' }}>
            {m.role==='user' ? (
              <div style={{ background:`rgba(${theme.rgbStr},.12)`,border:`1px solid rgba(${theme.rgbStr},.2)`,borderRadius:'18px 18px 4px 18px',padding:'12px 16px',maxWidth:'82%',color:'#F5ECD7',fontSize:15,lineHeight:1.55 }}>{m.content}</div>
            ) : (
              <div style={{ maxWidth:'92%' }}>
                {m.streaming ? (
                  <div style={{ color:'#d4c8a8',fontSize:15,lineHeight:1.7,whiteSpace:'pre-wrap' }}>{m.content}<span style={{ display:'inline-block',width:7,height:14,background:theme.primary,marginLeft:3,borderRadius:2,opacity:.7,animation:'blink .9s step-start infinite' }} /></div>
                ) : (
                  <div style={{ color:'#d4c8a8',fontSize:15,lineHeight:1.7,whiteSpace:'pre-wrap' }}>{m.content}</div>
                )}
              </div>
            )}
          </div>
        ...
        <div style={{ padding:'12px 20px 20px',maxWidth:520,margin:'0 auto',width:'100%',boxSizing:'border-box' }}>
          ...
          <div style={{ display:'flex',gap:8,alignItems:'flex-end',marginTop:chips.length>0?8:0 }}>
            <textarea ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter'&&!e.shiftKey){ e.preventDefault(); send() }}} placeholder="Type your answer…" rows={1} disabled={streaming}
              style={{ flex:1,background:'rgba(255,255,255,.04)',border:`1px solid rgba(${theme.rgbStr},.14)`,borderRadius:12,padding:'12px 15px',color:'#F5ECD7',fontSize:15,fontFamily:`'${font}',Georgia,serif`,resize:'none',outline:'none',lineHeight:1.5 }} />
