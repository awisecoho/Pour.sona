...
function NotFound() {
  return (
    ...
    >
      ...
      <div style={{ color: '#C9A84C', fontFamily: 'Georgia, serif', fontSize: 20 }}>
        Retailer not found
      </div>
      <div style={{ color: '#6a5a3a', fontFamily: 'Georgia, serif', fontSize: 15 }}>
        This QR code may be inactive or expired.
      </div>
    </div>
  )
}
...
function RecommendationCard({
  ...
  return (
    <div
      style={{
        background: 'linear-gradient(145deg,#0e0803,#0a1408)',
    ...
    >
      ...
      <div style={{ marginBottom: 16 }}>
        {products.map(
          (product: { name: string; why?: string; ratio?: number }, index: number) => (
            <div
              key={index}
              style={{ padding: '8px 0', borderBottom: '1px solid rgba(201,168,76,.07)' }}
            >
              <div style={{ color: '#F5ECD7', fontSize: 15 }}>
                {product.name}
                {product.ratio ? ` — ${product.ratio}%` : ''}
              </div>
              {product.why && (
                <div style={{ color: '#8a7a5a', fontSize: 13, marginTop: 2 }}>{product.why}</div>
              )}
            </div>
          )
        )}
      ...
      <div style={{ color: '#c8bfa8', fontSize: 15, lineHeight: 1.75, marginBottom: 16 }}>
        {rec.story}
      </div>
      <div
        style={{
      ...
      {ordered ? (
        <div
          style={{
            background: 'rgba(94,207,138,.1)',
            border: '1px solid rgba(94,207,138,.3)',
            borderRadius: 12,
            padding: '16px',
            textAlign: 'center',
          }}
        >
          ...
          <div style={{ color: '#5ecf8a', fontSize: 15 }}>
            Order placed! {retailer.name} will have it ready for you.
          </div>
        </div>
      ) : (
        <button
...
export default function CustomerPage({ params }: { params: { slug: string } }) {
  ...
  const send = async () => {
    ...
    setInput('')
    await streamChat(nextMessages.map((message) => ({ role: message.role, content: message.content })))
  }
  ...
  if (!started) {
    return (
      ...
      >
        ...
        >
          Let me help you find the perfect selection from our catalog — just answer a few quick
          questions.
        </div>
        <button
          onClick={start}
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <div style={{ color: '#3a2a0a', fontSize: 10, letterSpacing: '.15em' }}>
            POURSONA GUIDE
          </div>
        </div>
      </div>
        ...
        {messages.map((message, index) => (
          ...
          >
            {message.role === 'assistant' && (
              <div style={{ color: '#3a2a0a', fontSize: 9, letterSpacing: '.18em', marginBottom: 4 }}>
                POURSONA
              </div>
            )}
            <div
              style={{
                ...
                whiteSpace: 'pre-wrap',
                background:
                  message.role === 'user' ? 'rgba(201,168,76,.12)' : 'rgba(255,255,255,.04)',
                border:
                  message.role === 'user'
                    ? '1px solid rgba(201,168,76,.25)'
                    : '1px solid rgba(255,255,255,.06)',
                borderRadius:
                  message.role === 'user' ? '17px 17px 4px 17px' : '4px 17px 17px 17px',
                color: message.role === 'user' ? '#C9A84C' : '#c8bfa8',
              }}
            >
      ...
      {messages.filter((message) => message.role === 'user').length >= 2 && !rec && (
        <div
          style={{
            maxWidth: 640,
            width: '100%',
          ...
          <div style={{ display: 'flex', gap: 9, alignItems: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 9,
                padding: '11px 16px',
                color: input.trim() && !streaming ? '#0a0603' : '#3a2a0a',
