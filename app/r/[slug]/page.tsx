...
// This is what the customer sees when they scan the QR code
// URL: /r/ember-oak  or  /r/ironweed  or  /r/ridgeline
...
const stripRec = (t: string) => t.replace(/---RECOMMENDATION_START---[\s\S]*?---RECOMMENDATION_END---/g, '').trim()
const extractRec = (t: string): BlendRecommendation | null => {
  const m = t.match(/---RECOMMENDATION_START---([\s\S]*?)---RECOMMENDATION_END---/)
  if (!m) return null
...
function RecommendationCard({
  rec, retailer, sessionId, onOrder
}: {
  ...
  const handleOrder = async () => {
    setOrdering(true)
    const items = products.map((p: { name: string }) => ({
      name: p.name, size: 'Standard', price: 0, qty: 1
    }))
    await fetch('/api/order', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId, retailerId: retailer.id,
        items, blendName: rec.recommendationName,
      }),
    })
    setOrdering(false)
    setOrdered(true)
  ...
  return (
    <div style={{ background: 'linear-gradient(145deg,#0e0803,#0a1408)', border: '1px solid rgba(201,168,76,.3)', borderRadius: 18, padding: '24px 20px', marginTop: 20 }}>
      ...
      {ordered ? (
        <div style={{ background: 'rgba(94,207,138,.1)', border: '1px solid rgba(94,207,138,.3)', borderRadius: 12, padding: '16px', textAlign: 'center' }}>
        ...
        <button onClick={handleOrder} disabled={ordering} style={{
          ...
          border: 'none', cursor: ordering ? 'wait' : 'pointer',
          color: '#0a0603', fontSize: 13, fontWeight: 700, letterSpacing: '.15em'
        }}>
...
export default function CustomerPage({ params }: { params: { slug: string } }) {
  ...
  // Load retailer on mount
  useEffect(() => {
    async function init() {
      const r = await getRetailerBySlug(params.slug)
      if (!r) { setNotFound(true); setLoading(false); return }
      setRetailer(r)
      const sid = await createSession(r.id)
      setSessionId(sid)
      await logEvent(r.id, sid, 'scan', { slug: params.slug })
      setLoading(false)
    }
    init()
  }, [params.slug])
      ...
      while (true) {
        ...
        for (const line of dec.decode(value, { stream: true }).split('\n')) {
          ...
          try {
            ...
            if (parsed.done) {
              if (parsed.blendData) setRec(parsed.blendData)
              setMessages(prev => {
                const u = [...prev]
                u[u.length - 1] = { role: 'assistant', content: stripRec(full), streaming: false }
                return u
              })
            }
          } catch {}
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'linear-gradient(160deg,#0a0603,#0d1a0f)', fontFamily: 'Georgia, serif' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px 14px', maxWidth: 640, width: '100%', margin: '0 auto', alignSelf: 'stretch' }}>
        {messages.map((m, i) => (
          <div key={i} style={{ marginBottom: 16, display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            ...
            <div style={{
              ...
              background: m.role === 'user' ? 'rgba(201,168,76,.12)' : 'rgba(255,255,255,.04)',
              border: m.role === 'user' ? '1px solid rgba(201,168,76,.25)' : '1px solid rgba(255,255,255,.06)',
              borderRadius: m.role === 'user' ? '17px 17px 4px 17px' : '4px 17px 17px 17px',
              color: m.role === 'user' ? '#C9A84C' : '#c8bfa8',
            }}>
              {m.content === '' && m.streaming
                ? <span style={{ display: 'inline-flex', gap: 4 }}>{[0,1,2].map(i => <span key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: '#C9A84C', animation: `blink 1.2s ease-in-out ${i*.2}s infinite`, display: 'inline-block' }}/>)}</span>
                : <>{m.content}{m.streaming && m.content && <span style={{ display: 'inline-block', width: 2, height: '.85em', background: '#C9A84C', marginLeft: 2, verticalAlign: 'text-bottom', animation: 'cursor .8s ease-in-out infinite' }}/>}</>
              }
            </div>
          </div>
        ))}
        ...
        {ordered && (
          <div style={{ textAlign: 'center', marginTop: 24, color: '#5ecf8a', fontSize: 16 }}>
            Thank you! We hope you enjoy your selection. ✦
          </div>
        )}
      ...
      {messages.filter(m => m.role === 'user').length >= 2 && !rec && (
        <div style={{ maxWidth: 640, width: '100%', margin: '0 auto', padding: '0 14px 5px', alignSelf: 'stretch' }}>
          <button onClick={() => { setInput('I think you have enough — give me your recommendation.'); setTimeout(send, 50) }}
            disabled={streaming}
            style={{ background: 'transparent', border: '1px solid rgba(201,168,76,.22)', borderRadius: 18, padding: '6px 16px', color: '#6a5020', fontSize: 11, letterSpacing: '.12em', cursor: streaming ? 'default' : 'pointer', opacity: streaming ? .4 : 1 }}>
        ...
        <div style={{ borderTop: '1px solid rgba(201,168,76,.1)', padding: '11px 14px', background: 'rgba(0,0,0,.5)', backdropFilter: 'blur(10px)', flexShrink: 0, maxWidth: 640, width: '100%', margin: '0 auto', alignSelf: 'stretch' }}>
          <div style={{ display: 'flex', gap: 9, alignItems: 'flex-end' }}>
            <textarea value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder="Share your thoughts…" rows={1} disabled={streaming}
              style={{ flex: 1, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(201,168,76,.18)', borderRadius: 11, padding: '11px 14px', color: '#F5ECD7', fontFamily: 'Georgia, serif', fontSize: 16, resize: 'none', outline: 'none', minHeight: 44, opacity: streaming ? .6 : 1 }}
            />
