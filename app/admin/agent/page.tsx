  ...
  return (
    <div style={{ maxWidth: 880 }}>
      ...
      <Section title="Question Strategy" subtitle="How many questions, and which topics, before recommending.">
        ...
        <div style={{ color: '#4a3a1a', fontSize: 11, marginTop: 4, marginBottom: 20 }}>
          Resolved bounds for this retailer: <strong style={{ color: '#C9A84C' }}>{bounds.min}–{bounds.max} questions</strong>. The assistant stops asking sooner when it has enough.
        </div>
...
function Header({ bounds, category }: { bounds: { min: number; max: number }, category: CategoryTemplate }) {
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ color: '#C9A84C', fontSize: 10, letterSpacing: '.3em', textTransform: 'uppercase', marginBottom: 4 }}>Assistant Profile</div>
      <div style={{ color: '#F5ECD7', fontSize: 26, fontWeight: 700 }}>Your Agent</div>
      <div style={{ color: '#4a3a1a', fontSize: 13, marginTop: 6, lineHeight: 1.5, maxWidth: 640 }}>
        Tune how the guest-facing chat introduces itself, asks questions, and recommends. Fields left blank use the{' '}
        <strong style={{ color: '#C9A84C' }}>{category.vertical}</strong> category default. Current question range:{' '}
        <strong style={{ color: '#C9A84C' }}>{bounds.min}–{bounds.max}</strong>.
      </div>
    </div>
  )
}
...
function Section({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return (
    <div style={{ background: 'linear-gradient(145deg,#0e0b06,#0a0805)', border: '1px solid rgba(201,168,76,.15)', borderRadius: 14, padding: '24px 24px 8px', marginBottom: 18 }}>
      <div style={{ color: '#F5ECD7', fontSize: 15, fontWeight: 700, marginBottom: 4 }}>{title}</div>
      <div style={{ color: '#4a3a1a', fontSize: 12, marginBottom: 18 }}>{subtitle}</div>
      {children}
    </div>
  )
}
...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 18 }}>
      <label style={{ color: '#C9A84C', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>{label}</label>
      {hint && <div style={{ color: '#4a3a1a', fontSize: 11, marginBottom: 8, lineHeight: 1.5 }}>{hint}</div>}
      {children}
    </div>
  )
}
...
function ActionBar({ onSave, onReset, saving }: { onSave: () => void; onReset: () => void; saving: boolean }) {
  return (
    <div style={{ display: 'flex', gap: 12, marginTop: 8, position: 'sticky', bottom: 0, padding: '16px 0', background: 'linear-gradient(0deg,#080604,#080604 60%,transparent)' }}>
      <button onClick={onSave} disabled={saving} style={{
        flex: 1, padding: '14px', background: 'linear-gradient(135deg,#C9A84C,#a07830)', border: 'none', borderRadius: 10,
        color: '#0a0603', fontFamily: 'Georgia, serif', fontSize: 14, fontWeight: 700, cursor: saving ? 'wait' : 'pointer',
      ...
      <button onClick={onReset} disabled={saving} style={{
        ...
        color: '#e07070', fontFamily: 'Georgia, serif', fontSize: 13, cursor: saving ? 'wait' : 'pointer',
...
const inputStyle: React.CSSProperties = {
  ...
  border: '1px solid rgba(201,168,76,.15)', borderRadius: 8, color: '#F5ECD7',
...
function Select<T extends string>({ value, onChange, options }: { value: T; onChange: (v: T) => void; options: { value: T; label: string }[] }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value as T)} style={inputStyle}>
      {options.map((o) => <option key={o.value} value={o.value} style={{ background: '#0a0603' }}>{o.label}</option>)}
    </select>
  )
}

function TagInput({ values, onChange, placeholder, tagColor = '#C9A84C' }: { values: string[]; onChange: (v: string[]) => void; placeholder?: string; tagColor?: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {themes.map((t) => {
        ...
        return (
          <label key={t.id} style={{ display: 'flex', gap: 12, padding: '12px 14px', borderRadius: 10, border: `1px solid ${on ? 'rgba(201,168,76,.35)' : 'rgba(201,168,76,.1)'}`, background: on ? 'rgba(201,168,76,.06)' : 'transparent', cursor: 'pointer' }}>
            <input type="checkbox" checked={on} onChange={() => toggle(t.id)} style={{ marginTop: 3, accentColor: '#C9A84C' }} />
            <div style={{ flex: 1 }}>
              <div style={{ color: on ? '#C9A84C' : '#F5ECD7', fontSize: 13, fontWeight: 600, marginBottom: 2 }}>{t.label}</div>
              <div style={{ color: '#4a3a1a', fontSize: 11, lineHeight: 1.5, fontStyle: 'italic' }}>
...
function RulesEditor({ rules, onChange }: { rules: RecommendationRule[]; onChange: (v: RecommendationRule[]) => void }) {
  ...
  return (
    <div>
      {rules.map((r, i) => (
        <div key={i} style={{ padding: 14, borderRadius: 10, border: '1px solid rgba(201,168,76,.12)', background: 'rgba(255,255,255,.02)', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ color: '#4a3a1a', fontSize: 11 }}>Rule {i + 1}</span>
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ color: '#6a5a3a', fontSize: 11, marginBottom: 6 }}>When the guest says (or signals):</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              <div style={{ color: '#6a5a3a', fontSize: 11, marginBottom: 6 }}>Prioritize categories</div>
              <TagInput values={r.prioritize_categories || []} onChange={(v) => update(i, { prioritize_categories: v })} placeholder="Lager, Pilsner" />
            </div>
            <div>
              <div style={{ color: '#6a5a3a', fontSize: 11, marginBottom: 6 }}>Avoid categories</div>
              <TagInput values={r.avoid_categories || []} onChange={(v) => update(i, { avoid_categories: v })} placeholder="Imperial IPA" tagColor="#e07070" />
            </div>
          </div>
        </div>
      ))}
      <button onClick={add} style={{ padding: '10px 16px', background: 'rgba(201,168,76,.08)', border: '1px dashed rgba(201,168,76,.3)', borderRadius: 8, color: '#C9A84C', fontFamily: 'Georgia, serif', fontSize: 12, cursor: 'pointer' }}>
        + Add a rule
      </button>
    </div>
  )
