    ...
    <div>
          ...
          {data.map((d, i) => {
            ...
            return (
              <g key={d.day}>
                ...
                {d.orders > 0 && (
                  <rect x={x} y={72 - ordH} width={barW} height={ordH} fill="rgba(97,42,134,0.65)" rx={1} />
                )}
              </g>
            )
          })}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 2 }}>
        {data.filter((_, i) => i === 0 || i === data.length - 1 || i % labelEvery === 0).map(d => (
          <span key={d.day} style={{ color: '#3A3450', fontSize: 9 }}>{d.day.slice(5)}</span>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#3A3450', fontSize: 10 }}>
          <span style={{ width: 10, height: 10, borderRadius: 2, background: 'rgba(255,255,255,0.08)', display: 'inline-block' }} />Sessions
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#D67A31', fontSize: 10 }}>
          <span style={{ width: 10, height: 10, borderRadius: 2, background: 'rgba(97,42,134,0.65)', display: 'inline-block' }} />Orders
        </span>
      </div>
    </div>
  )
...
function OnboardingChecklist({ retailerSlug, onDismiss }: { retailerSlug: string; onDismiss: () => void }) {
  ...
  return (
    <div style={{ background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.25)', borderRadius: 14, padding: '28px 28px 20px', marginBottom: 32 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          <div style={{ color: '#D67A31', fontSize: 10, letterSpacing: '.25em', textTransform: 'uppercase', marginBottom: 4 }}>Getting started</div>
          <div style={{ color: '#F5F2E8', fontSize: 18, fontWeight: 700 }}>You&apos;re almost live</div>
          <div style={{ color: '#3A3450', fontSize: 13, marginTop: 4 }}>3 quick steps to start guiding guests.</div>
        </div>
        <button onClick={onDismiss} style={{ background: 'none', border: 'none', color: '#3A3450', cursor: 'pointer', fontSize: 18, padding: '0 4px', lineHeight: 1 }} title="Dismiss">×</button>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
        {CHECKLIST.map((step, i) => (
          <div key={step.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: 'rgba(255,255,255,.03)', borderRadius: 10, border: '1px solid rgba(97,42,134,.08)' }}>
            <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(97,42,134,.1)', border: '1px solid rgba(97,42,134,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#D67A31', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
              {i + 1}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ color: '#F5F2E8', fontSize: 13, fontWeight: 700 }}>{step.label}</div>
              <div style={{ color: '#3A3450', fontSize: 12, marginTop: 2 }}>{step.desc}</div>
            </div>
            {step.href ? (
              <Link href={step.href} style={{ padding: '6px 14px', background: 'rgba(97,42,134,.1)', border: '1px solid rgba(97,42,134,.2)', borderRadius: 8, color: '#D67A31', fontSize: 11, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                {step.cta}
              </Link>
            ) : (
              <a href={`/r/${retailerSlug}`} target="_blank" rel="noopener noreferrer" style={{ padding: '6px 14px', background: 'rgba(97,42,134,.1)', border: '1px solid rgba(97,42,134,.2)', borderRadius: 8, color: '#D67A31', fontSize: 11, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                Preview Guide →
              </a>
            )}
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ flex: 1, height: 3, background: 'rgba(97,42,134,.1)', borderRadius: 2 }}>
          <div style={{ width: `${(completedCount / CHECKLIST.length) * 100}%`, height: '100%', background: '#D67A31', borderRadius: 2, transition: 'width .3s' }} />
        </div>
        <div style={{ color: '#3A3450', fontSize: 11, whiteSpace: 'nowrap' }}>{completedCount}/{CHECKLIST.length} done</div>
      </div>
    </div>
  )
}
...
function Stat({ label, value, sub, color = '#D67A31' }: any) {
  return (
    <div style={{ background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.15)', borderRadius: 14, padding: '24px 20px' }}>
      <div style={{ color: '#3A3450', fontSize: 10, letterSpacing: '.2em', textTransform: 'uppercase', marginBottom: 8 }}>{label}</div>
      <div style={{ color, fontSize: 36, fontWeight: 700, lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ color: '#6A6080', fontSize: 12, marginTop: 6 }}>{sub}</div>}
    </div>
  )
}
...
export default function Dashboard() {
  ...
  if (retailerLoading || loading) return <div style={{ color: '#D67A31' }}>Loading…</div>
  if (message) return <div style={{ color: '#F5F2E8', fontFamily: "var(--font-inter), system-ui, sans-serif" }}>{message}</div>
  ...
  const btn = (v?: 'outline'): React.CSSProperties => ({ padding: '7px 14px', border: 'none', borderRadius: 7, cursor: 'pointer', fontFamily: "var(--font-inter), system-ui, sans-serif", fontSize: 12, fontWeight: 700, background: v === 'outline' ? 'transparent' : 'linear-gradient(135deg,#D67A31,#612A86)', color: v === 'outline' ? '#D67A31' : '#12111A', ...(v === 'outline' ? { border: '1px solid rgba(97,42,134,.25)' } : {}) })
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ color: '#D67A31', fontSize: 10, letterSpacing: '.3em', textTransform: 'uppercase', marginBottom: 4 }}>Dashboard</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {retailer?.logo_url
            ? <img src={retailer.logo_url} alt="" style={{ height: 36, objectFit: 'contain', borderRadius: 4 }} />
            : <span style={{ fontSize: 28 }}>{icon}</span>}
          <div style={{ color: '#F5F2E8', fontSize: 22, fontWeight: 700, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{retailer?.name}</div>
        </div>
        <div style={{ color: '#3A3450', fontSize: 12, marginTop: 6, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <span style={{ textTransform: 'capitalize' }}>{retailer?.vertical}</span>
          {retailer?.location && <span>{retailer.location}</span>}
          <a href={'/r/' + retailer?.slug} target="_blank" style={{ color: '#D67A31', textDecoration: 'none' }}>↗ /r/{retailer?.slug}</a>
        </div>
      </div>
      ...
      <div className="dash-date-row">
        <span style={{ color: '#3A3450', fontSize: 11 }}>{rangeLabel}</span>
      ...
      {/* AI usage this month */}
      {aiUsage && (
        <div style={{ background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.15)', borderRadius: 14, padding: '16px 20px', marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10, flexWrap: 'wrap', gap: 6 }}>
            <div style={{ color: '#F5F2E8', fontSize: 13, fontWeight: 700 }}>AI Usage This Month</div>
            <div style={{ color: aiUsage.pct >= 100 ? '#e07070' : aiUsage.pct >= 80 ? '#D67A31' : '#3A3450', fontSize: 12 }}>
              ${aiUsage.costUsd.toFixed(2)} / ${aiUsage.budgetUsd.toFixed(0)} · {aiUsage.pct}%
            </div>
          </div>
          <div style={{ height: 6, background: 'rgba(255,255,255,.06)', borderRadius: 3, overflow: 'hidden' }}>
            <div style={{ width: aiUsage.pct + '%', height: '100%', background: aiUsage.pct >= 100 ? '#e07070' : aiUsage.pct >= 80 ? '#D67A31' : '#5ecf8a', transition: 'width .3s' }} />
          </div>
          {aiUsage.pct >= 100 && (
      ...
      {/* Daily trend chart */}
      {daily.length > 1 && (
        <div style={{ background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.15)', borderRadius: 14, padding: '20px 20px', marginBottom: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 6 }}>
            <div style={{ color: '#F5F2E8', fontSize: 13, fontWeight: 700 }}>Daily Activity</div>
            <div style={{ color: '#3A3450', fontSize: 10 }}>{range?.chartFrom} → {range?.chartTo}</div>
          </div>
      ...
      <div style={{ background: 'linear-gradient(145deg,#1C1A2A,#161423)', border: '1px solid rgba(97,42,134,.15)', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ padding: '18px 20px', borderBottom: '1px solid rgba(97,42,134,.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ color: '#F5F2E8', fontSize: 14, fontWeight: 700 }}>Recent Sessions</div>
          <div style={{ color: '#3A3450', fontSize: 11 }}>{retailer?.name}</div>
        </div>
        {recent.length === 0 ? (
          <div style={{ padding: '32px 20px', textAlign: 'center', color: '#3A3450', fontSize: 13 }}>
          ...
          <>
            ...
            <div className="dash-sessions-table">
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(97,42,134,.08)' }}>
                    {['Session','Status','Date'].map(h => <th key={h} style={{ padding: '10px 20px', textAlign: 'left', color: '#3A3450', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', fontWeight: 400 }}>{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {recent.map((s: any) => (
                    <tr key={s.id} style={{ borderBottom: '1px solid rgba(97,42,134,.05)' }}>
                      <td style={{ padding: '12px 20px', color: '#6A6080', fontSize: 12 }}>{s.id.substring(0,8)}…</td>
                      <td style={{ padding: '12px 20px' }}>
                        <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11,
                          background: s.order_status==='ordered'?'rgba(94,207,138,.15)':s.order_status==='recommended'?'rgba(97,42,134,.15)':'rgba(255,255,255,.05)',
                          color: s.order_status==='ordered'?'#5ecf8a':s.order_status==='recommended'?'#D67A31':'#6A6080'
                        }}>{s.order_status}</span>
                      </td>
                      <td style={{ padding: '12px 20px', color: '#6A6080', fontSize: 12 }}>
            ...
            <div className="dash-sessions-cards" style={{ padding: '12px' }}>
              {recent.map((s: any) => (
                <div key={s.id} style={{ padding: '14px 16px', background: 'rgba(255,255,255,.02)', border: '1px solid rgba(97,42,134,.07)', borderRadius: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ color: '#6A6080', fontSize: 12 }}>{s.id.substring(0,8)}…</span>
                    <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11,
                      background: s.order_status==='ordered'?'rgba(94,207,138,.15)':s.order_status==='recommended'?'rgba(97,42,134,.15)':'rgba(255,255,255,.05)',
                      color: s.order_status==='ordered'?'#5ecf8a':s.order_status==='recommended'?'#D67A31':'#6A6080'
                    }}>{s.order_status}</span>
                  </div>
                  <div style={{ color: '#3A3450', fontSize: 11 }}>
                    {new Date(s.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}
                  </div>
                </div>
              ))}
